...
public class Dearator extends WaterSteamComponent implements Connectable, UIReadable {
    ...
    public void update() {
        ...
        waterTemperature += ((NPPSim.tables.getSteamEnthalpyByTemperature(steamTemperature)
                - NPPSim.tables.getSteamEnthalpyByTemperature(waterTemperature)) * steamMass)
                / (specificHeatWater * waterMass);
        specificHeatWater = NPPMath.calculateSpecificHeatWater(waterTemperature);
        deltaEnergy += (NPPSim.tables.getSteamEnthalpyByTemperature(waterTemperature)
                - NPPSim.tables.getSteamEnthalpyByTemperature(oldWaterTemp)) * steamMass; // correct for difference in
                                                                                          // new steamTemp and new
                                                                                          // waterTemp after energy
                                                                                          // transfer
        ...
        waterVolume = waterMass * specificDensityWater;
        steamVolume = volume - waterVolume;
        deltaSteamMass = steamVolume / NPPSim.tables.getSteamDensityByPressure(potentialPressure)
