...
public class Core extends Component {
    ...
    public void update() {
        ...
        kEff = Math.pow(neutronCount / previousNeutronCount, 2.0); // to the power of 2 because 2 simulation cycles in 1
                                                                   // neuton lifetime cycle
        ...
        for (short x = 0; x < coreArray.size(); x++) {
            for (short y = 0; y < coreArray.get(x).size(); y++) {
                ...
                Double[][] neighborChannels = new Double[8][2]; // an array of neighboring positions in a 9x9 grid
                                                                // omitting the 'self' position at the center starting
                ...
                for (short c = 0; c < 8; c++) {
                    ...
                    if (neighborXY[0] < 0 || neighborXY[0] > 55 || neighborXY[1] < 0 || neighborXY[1] > 55) { // continue
                                                                                                              // if a
                                                                                                              // neighboring
                                                                                                              // channel
                                                                                                              // would
                    ...
                    neighborCount++;
                    neighborChannels[c] = nextGenerationCount.get(neighborXY[0]).get(neighborXY[1]); // finally assign
                                                                                                     // address of
                                                                                                     // neighbor's
                                                                                                     // neutron count
                                                                                                     // array for next
     ...
     * @return coordinates {x, y} for neighboring channel depending on relative
     *         position
     */
    ...
    private ArrayList<List<String>> getCoreData() {
        ...
        try {
            BufferedReader reader = new BufferedReader(
                    new InputStreamReader((getClass().getResourceAsStream("/data/core_map.csv"))));
