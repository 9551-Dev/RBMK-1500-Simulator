...
public class SteamTables {
    ...
    public SteamTables() {
        try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(
                    (getClass().getResourceAsStream("/data/saturated_by_temperature_V1.4.json"))));
            String jsonString = "";
            String line;
        ...
        for (int y = 778750; y >= 32; y--) {
            for (int i = 0; i < tables.length(); i++) {
                if (tables.getJSONArray(i).getDouble(3) < y / 10000.0) {
                    ...
                    steamPressureByDensityMap.put(y, tables.getJSONArray(i).getDouble(1)
                            + (y / 10000.0 - tables.getJSONArray(i).getDouble(3)) * perDens);
                    break;
                }
            }
        }
        for (int y = 17; y <= 220640; y++) {
            for (int i = 0; i < tables.length(); i++) {
                if (tables.getJSONArray(i).getDouble(1) > y / 10000.0) {
                    ...
                    steamDensityByPressureMap.put(y, tables.getJSONArray(i).getDouble(3)
                            + (y / 10000.0 - tables.getJSONArray(i).getDouble(1)) * perPress);
                    break;
                }
            }
            for (int i = 0; i < tables.length(); i++) {
                if (tables.getJSONArray(i).getDouble(1) > y / 10000.0) {
                    ...
                    steamTemperatureByPressureMap.put(y, tables.getJSONArray(i).getDouble(0)
                            + (y / 10000.0 - tables.getJSONArray(i).getDouble(1)) * perPress);
                    break;
                }
            }
        ...
        for (int y = 629; y <= 20483; y++) {
            for (int i = 0; i < tables.length(); i++) {
                if (tables.getJSONArray(i).getDouble(7) > y / 10.0) {
                    ...
                    waterTemperatureByEnthalpyMap.put(y, tables.getJSONArray(i).getDouble(0)
                            + (y / 10.0 - tables.getJSONArray(i).getDouble(7)) * perEnthalpy);
                    break;
                }
            }
        }
     ...
     * double dEnthalpy = tables.getJSONArray(i).getDouble(8) -
     * tables.getJSONArray(i - 1).getDouble(8);
     * double dTemp = tables.getJSONArray(i).getDouble(0) - tables.getJSONArray(i -
     * 1).getDouble(0);
     * double perEnthalpy = dTemp / dEnthalpy;
     * return tables.getJSONArray(i).getDouble(0) + (Enthalpy -
     * tables.getJSONArray(i).getDouble(8)) * perEnthalpy;
     * }
     * }
     * return 0.0;
     * }
