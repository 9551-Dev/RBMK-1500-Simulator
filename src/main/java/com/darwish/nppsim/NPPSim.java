...
public class NPPSim {
    ...
    public NPPSim() {
        ...
        // initialize condensate pumps and dearators TODO this is greatly simplified
        // needs reworking
        ...
        for (int i = 0; i < 3; i++) {
            condensate1B
                    .add(new Pump(2975, 0.417f, 1.766, 0.2157463, 30, 40, 267, condensateHeader1, condensateHeader));
        }
        for (int i = 0; i < 3; i++) {
            condensate2B
                    .add(new Pump(2975, 0.417f, 1.766, 0.2157463, 30, 40, 267, condensateHeader2, condensateHeader));
        }
        ...
        condensateHeader.setSources(new Pump[] { condensate1B.get(0), condensate1B.get(1), condensate1B.get(2),
                condensate2B.get(0), condensate2B.get(1), condensate2B.get(2) });
        for (int i = 0; i < 4; i++) {
        ...
        // auxiliaryFWSuctionHeader = new SimpleSuctionHeader(new Connectable[]
        // {dearators.get(0), dearators.get(1), dearators.get(2), dearators.get(3)});
        fwSuctionHeader = new SimpleSuctionHeader(
                new Connectable[] { dearators.get(0), dearators.get(1), dearators.get(2), dearators.get(3) }, 1);
        ...
        for (int i = 0; i < 7; i++) {
            mainFeedwaterPumps
                    .add(new Pump(2982, 0.458f, 8.9, 0.14709975, 30, 40, 833, fwSuctionHeader, mainFWPressureHeader)); // 9.5
        }
        for (int i = 0; i < 6; i++) {
            auxFeedwaterPumps.add(
                    new Pump(2970, 0.069f, 8.6, 0.08825985, 30, 40, 133, fwSuctionHeader, auxiliaryFWPressureHeader)); // 9.1
        }
        mainFWPressureHeader.setSources(new Pump[] { mainFeedwaterPumps.get(0), mainFeedwaterPumps.get(1),
                mainFeedwaterPumps.get(2), mainFeedwaterPumps.get(3), mainFeedwaterPumps.get(4),
                mainFeedwaterPumps.get(5), mainFeedwaterPumps.get(6) });
        auxiliaryFWPressureHeader
                .setSources(new Pump[] { auxFeedwaterPumps.get(0), auxFeedwaterPumps.get(1), auxFeedwaterPumps.get(2),
                        auxFeedwaterPumps.get(3), auxFeedwaterPumps.get(4), auxFeedwaterPumps.get(5) });
        for (int i = 0; i < 3; i++) {
        ...
        updateThread = new Thread(() -> {
            try {
                while (run) {
                    ...
                    // RULE OF THUMB: objects from "Components" file should be updated before their
                    // sources/drains if possible
                    sdv_c.forEach(valve -> {
                        valve.update();
                    });
                    sdv_a.forEach(valve -> {
                    ...
                    // System.out.println(condensateHeader.pressure + " " +
                    // condensateHeader1.pressure);
        ...
        printThread = new Thread(() -> {
            try {
                while (run) {
                    ...
                    // System.out.println("Average time step: " + timeStepLengthCumulative /
                    // updateCount + " " + updateCount); //used sometimes for debugging
                    if (updateCount > 20) {
                        if (timeToSleep[0] > 500) {
                            timeToSleep[0] -= 50;
                        }
                        System.out.println(
                                "WARNING: Timesteps = " + updateCount + ". UI Wait time will be " + timeToSleep[0]);
                    }
                    updateCount = 0;
